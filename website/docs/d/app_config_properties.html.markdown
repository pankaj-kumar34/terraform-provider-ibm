...
Provides a read-only data source for `properties`. You can then reference the fields of the data source in other resources within the same configuration using interpolation syntax.
...
- `id` - The unique identifier of the PropertiesList.
- `properties` - Array of properties. Nested `properties` blocks have the following structure:
